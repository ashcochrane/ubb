...
1. **Full-pipeline seam test (mandatory before any flip):** DONE — `apps/billing/gating/tests/test_e2e_seam.py::TestEnforcementSeam` drives the whole flag-gated spine through the real endpoints under one flip (start-gate allows → live counter decrements → crossing returns 200 `stop` and is still charged → start-gate blocks a new run → top-up clears the flag → start-gate allows again), plus an `off`-tenant control proving the pipeline is gated by the single flag. (Durable suspend/un-suspend: `test_p6b_suspend`; per-run cap fan-out: `test_p6_fanout`.)
2. **Load / latency profile (the synchronous hot-path cost).** For an `enforcing` tenant, `record_usage` adds **two Redis ops** — one Lua `EVAL` (live debit, `SEED_AND_DECR`/`SPEND_INCR`) + one `GET` (stop-flag read) — plus the existing synchronous pricing/`accumulate_cost`. The start-gate (`pre_check`) adds one `COUNT(active runs)` query + one Redis `GET`. An `off` tenant adds **zero** (every path short-circuits on `mode=="off"` before touching Redis). All Redis ops are **fail-open**: `record_usage_debit`/`read_stop` swallow Redis errors and return `None`/not-stopped, so a Redis outage degrades to "no synchronous enforcement" with the durable start-gate + async suspend as the backstop (and the `reconcile`/`drift_spike` log surfaces it). Before enabling at scale: set a p99 SLO on `record_usage`, load-test at the tenant's peak event rate, and alert on `live_ledger.drift_spike` + Redis availability. (No code SLO is enforced in-process — this is an operational gate.)
...
- **Postpaid un-suspend is reconcile-latency-bounded (P6b).** A prepaid owner recovers instantly on top-up (`credit()` un-suspends, gated on the *durable* wallet). A postpaid `budget_exceeded` owner has no synchronous recovery hook — it un-suspends on the next `reconcile_live_ledgers` tick (hourly) once the new-month spend is under cap. `reconcile_live_ledgers` enumerates suspended-for-budget owners unconditionally (not just current-month-usage owners) so a start-gate-blocked owner is never stranded. Recovery latency ≤ one reconcile cycle.
