...
┌──────────────────────────────────────────────────────────────┐
│                     Composition Layer                         │
│              (optional, only for tenants with both)           │
│                                                              │
│   POST /api/v1/usage  →  metering.record + billing.debit    │
│   POST /api/v1/pre-check → metering.estimate + billing.check│
└──────────┬──────────────────────────────────────┬────────────┘
           │                                      │
           │         ┌──────────┐                 │
           │         │Event Bus │                 │
...
│ Calculates costs           │  │ Wallets, payments, invoices   │
│ Pricing rules              │  │ Gating (balance-based)        │
│                            │  │                               │
│ Knows nothing about money  │  │ Knows nothing about how costs │
...
- The **Composition Layer** imports from both products — provides combined endpoints for tenants using both.
- The **Event Bus** handles async side effects only. The critical financial path (debit/credit wallet) uses direct service calls through the composition layer.
...
GET    /metering/analytics/usage            # Breakdown by provider/type/customer
GET    /metering/analytics/revenue          # Revenue breakdown with markup
```
...
### Composition (`/api/v1/`) — requires both `"metering"` and `"billing"`

```
POST   /usage                               # Record + debit wallet
POST   /pre-check                           # Estimate cost + check balance
```

These match the current API paths for backwards compatibility.
...
```python
# composition/api/endpoints.py

@router.post("/usage")
def record_and_debit(request, payload):
    result = usage_service.record_usage(...)          # metering
    debit = wallet_service.debit(                     # billing
        customer_id, result.cost_micros,
        reference=str(result.event_id)
    )
    return {
        "event_id": result.event_id,
        "cost_micros": result.cost_micros,
        "balance_after_micros": debit.new_balance,
    }

@router.post("/pre-check")
def estimate_and_check(request, payload):
    cost = usage_service.estimate_cost(...)           # metering
    check = gating_service.pre_check(                 # billing
...
ubb-platform/
  ...
  apps/
    ...
    composition/
      api/
        endpoints.py          # /usage, /pre-check (calls both products)
        schemas.py
      apps.py
```
...
- Create `metering/api/`, `billing/api/`, `composition/api/`
- Move existing endpoints into their product's API module
- Old URL paths served by composition layer (backwards compatible)
- New `/metering/` and `/billing/` namespaced paths available
...
- Update SDK to support both combined and individual product APIs
...
| API structure | Each product owns its own API + composition layer | No if/else in endpoints. Products fully independent. Composition layer provides backwards-compatible combined endpoints. |
