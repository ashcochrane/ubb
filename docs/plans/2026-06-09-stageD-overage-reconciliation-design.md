...
- **I3 — Wrong-basis reuse errors loudly.** If, under the wallet lock, an existing `usage_deduction:{event_id}` debit is found whose **amount** differs from the event's billed basis, it is **not** silently treated as done — it is logged + alerted as a ledger anomaly. (The wallet is already pinned by the `(wallet, key)` lookup, and `WalletTransaction` has no currency column, so amount is the meaningful basis check.)
