...
| **Admission control** | `admission_control` | **kernel** | a bound on how fast work may enter is a property of the unit of work's admission, consulted by the composition layer for **every** tenant, before the money-shaped verdict; its one setting moved to the tenant row with it (#462). It says nothing about supplier cost and is never spend protection; a close is never subject to it. This ADR records only that the kernel's half comes first — the sequence a start runs is behaviour, and the billing glossary's **Start-gate** entry (`ubb-platform/apps/billing/CONTEXT.md`) is where it is defined |
