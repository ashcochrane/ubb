"""Task 4 / #111: the hold lane — the atomic accept-time hold (Lua) for the
async ingest path, an op of the ONE live counter (``LiveCounter.hold``).

The hold rides the SAME owner-keyed prepaid balance / postpaid spend counter
and cooperative customer-wide stop flag that the synchronous
``LiveCounter.debit`` path maintains — one module, one counter, so a hold
taken here is visible to (and raced against) the synchronous path.
...
def _flush_redis_keys():
    # Same idiom as the other Tier-2 gating tests (test_live_counter.py
    # etc): cache.clear() FLUSHDBs the dedicated test Redis db (see root
    # conftest.py), which also wipes the raw live-counter keys written via
    # the module's _client() (a separate raw connection to the SAME db — not
    # routed through django's cache API/prefixing).
    cache.clear()
    yield
    cache.clear()
...
def test_hold_decrements_live_balance(enforced_prepaid_tenant, funded_owner):
    out = LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant,
                              [_item(2_920_000)])
    assert out[0]["held"] and not out[0]["stop"]
    assert Door.balance(funded_owner.id) == 17_080_000


def test_floor_crossing_sets_stop_but_holds(enforced_prepaid_tenant, funded_owner):
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(19_600_000)])
    out = LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(500_000)])
...
def test_acquire_never_rejects_far_beyond_balance(enforced_prepaid_tenant, funded_owner):
    ...
    assert out[0]["stop"] is True
    assert out[0]["stop_scope"] == "customer"
    assert Door.balance(funded_owner.id) == 20_000_000 - 999_000_000_000
...
def test_second_crossing_while_flag_set_does_not_spam(enforced_prepaid_tenant, funded_owner):
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(19_600_000)])
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(500_000)])  # crosses
    assert OutboxEvent.objects.filter(event_type="stop.fired").count() == 1
...
def test_flag_only_delete_does_not_re_emit_but_a_real_clear_re_arms(enforced_prepaid_tenant, funded_owner):
    ...
    Door.delete_stop(funded_owner.id)
    ...
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(500_000)])
...
def test_publish_failure_does_not_raise_into_acquire(enforced_prepaid_tenant, funded_owner, monkeypatch):
    ...
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(19_600_000)])
    out = LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(500_000)])  # crosses
    assert out[0]["held"] is True
    assert out[0]["stop"] is True
...
def test_settle_delta_credits_overhold(enforced_prepaid_tenant, funded_owner):
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(2_500_000)])
    LiveCounter.settle(funded_owner.id, enforced_prepaid_tenant, 300_000)  # exact was 2_200_000
    assert Door.balance(funded_owner.id) == 17_800_000
...
def test_settle_negative_delta_debits_further(enforced_prepaid_tenant, funded_owner):
    ...
    LiveCounter.settle(funded_owner.id, enforced_prepaid_tenant, -100_000)
    assert Door.balance(funded_owner.id) == 17_400_000


def test_release_fully_credits_back(enforced_prepaid_tenant, funded_owner):
    LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(5_000_000)])
    assert Door.balance(funded_owner.id) == 15_000_000
    LiveCounter.release(funded_owner.id, enforced_prepaid_tenant, 5_000_000)
    assert Door.balance(funded_owner.id) == 20_000_000
...
def test_postpaid_incr_increases_spend_not_decreases(enforced_postpaid_tenant, postpaid_owner):
    ...
    backwards)."""
    out = LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant, [_item(4_000_000)])
    assert out[0]["held"]
    assert Door.spend(postpaid_owner.id) == 4_000_000
    LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant, [_item(1_000_000)])
    assert Door.spend(postpaid_owner.id) == 5_000_000
...
def test_postpaid_settle_delta_lowers_live_spend(enforced_postpaid_tenant, postpaid_owner):
    ...
    over-hold (LiveCounter.credit no-ops for postpaid, so settle needs
    its own direct livespend adjustment — otherwise every over-estimate
    permanently inflates the counter: the postpaid reconcile only MAX-merges
    and can never lower it back)."""
    LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant, [_item(2_500_000)])
    assert Door.spend(postpaid_owner.id) == 2_500_000
    LiveCounter.settle(postpaid_owner.id, enforced_postpaid_tenant, 300_000)  # exact was 2_200_000
    assert Door.spend(postpaid_owner.id) == 2_200_000
...
def test_postpaid_settle_negative_delta_raises_live_spend(enforced_postpaid_tenant, postpaid_owner):
    ...
    LiveCounter.settle(postpaid_owner.id, enforced_postpaid_tenant, -100_000)
    assert Door.spend(postpaid_owner.id) == 2_600_000


def test_postpaid_release_restores_live_spend(enforced_postpaid_tenant, postpaid_owner):
    LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant, [_item(4_000_000)])
    LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant, [_item(1_000_000)])
    assert Door.spend(postpaid_owner.id) == 5_000_000
    # Full release of the second hold (duplicate/failed append) -> back to
    # the pre-hold value.
    LiveCounter.release(postpaid_owner.id, enforced_postpaid_tenant, 1_000_000)
    assert Door.spend(postpaid_owner.id) == 4_000_000
...
def test_postpaid_prior_month_acquire_skips_livespend(
    ...
    live spend counter (I9 parity with LiveCounter.debit) — it is still
    held."""
    out = LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant,
                              [_item(4_000_000, effective_at=_prior_month_instant())])
    assert out[0]["held"] is True
    assert Door.spend(postpaid_owner.id) is None
...
def test_postpaid_settle_of_prior_month_event_skips_livespend_adjustment(
        enforced_postpaid_tenant, postpaid_owner):
    """settle() must mirror hold()'s prior-month skip: settling an event
    whose effective_at falls in a PRIOR month must not adjust the current
    month's (unrelated) livespend counter."""
    LiveCounter.hold(postpaid_owner.id, enforced_postpaid_tenant, [_item(3_000_000)])
    assert Door.spend(postpaid_owner.id) == 3_000_000

    LiveCounter.settle(postpaid_owner.id, enforced_postpaid_tenant, 500_000,
                       effective_at=_prior_month_instant())
    assert Door.spend(postpaid_owner.id) == 3_000_000  # unchanged
...
def test_enforcement_off_is_noop_fail_open(funded_owner):
    ...
    out = LiveCounter.hold(funded_owner.id, off_tenant, [_item(999_000_000)])
    assert out[0]["held"] and not out[0]["stop"]
    assert Door.balance(funded_owner.id) is None


def test_fail_open_on_redis_error_holds_every_item(enforced_prepaid_tenant, funded_owner, monkeypatch):
    # _client is the ONE monkeypatch seam for Redis-down tests (D4) — the
    # old hold_service name-rebinding wrinkle died with the consolidation.
    def _boom():
        raise ConnectionError("redis down")

    monkeypatch.setattr("apps.billing.gating.services.live_counter._client", _boom)
    out = LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant,
...
def test_acquire_threshold_query_count_does_not_scale_with_batch_size(
        enforced_prepaid_tenant, funded_owner):
    """Before the fix, the crossing check (and its ORM lookup) ran ONCE PER
    HELD ITEM inside the hold loop -- so a 50-item batch issued ~50x the ORM
    queries of a 1-item batch. The threshold is now resolved ONCE per hold()
    call and compared per item in plain Python, so the query count must be
    ...
    with CaptureQueriesContext(connection) as ctx_one:
        out1 = LiveCounter.hold(funded_owner.id, enforced_prepaid_tenant, [_item(10_000)])
    assert out1[0]["held"]
    one_item_queries = len(ctx_one.captured_queries)

    with CaptureQueriesContext(connection) as ctx_many:
        out50 = LiveCounter.hold(
...
def test_batch_pipelines_multiple_items_one_call(enforced_prepaid_tenant, funded_owner):
    ...
    assert Door.balance(funded_owner.id) == 20_000_000 - 3_000_000
...
def test_concurrent_holds_at_floor_race(enforced_prepaid_tenant, funded_owner):
    ...
    then fail invisibly-to-the-thread and get masked by the hold's
    ...
    def go():
        try:
            results.extend(LiveCounter.hold(
    ...
    [t.start() for t in ts]
    [t.join() for t in ts]
    assert Door.balance(funded_owner.id) == 20_000_000 - 30_000_000
