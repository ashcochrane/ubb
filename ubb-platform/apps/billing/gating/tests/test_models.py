...
class TestCustomerSpendPool:
    def test_defaults(self):
        from apps.billing.gating.models import CustomerSpendPool
        t = Tenant.objects.create(name="T")
        cfg = CustomerSpendPool.objects.create(tenant=t, cap_micros=1_000_000_000)
    ...
    def test_tenant_default_and_customer_override_coexist(self):
        ...
        CustomerSpendPool.objects.create(tenant=t, customer=None, cap_micros=1_000)
        CustomerSpendPool.objects.create(tenant=t, customer=c, cap_micros=2_000)
        assert CustomerSpendPool.objects.filter(tenant=t).count() == 2
