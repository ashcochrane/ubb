...
def test_close_rolls_seats_into_one_business_invoice():
    ...
    for seat, key, amt in [(s1, "i1", 800_000), (s2, "i2", 300_000)]:
        ev = Posting.objects.create(tenant=t, customer=seat, idempotency_key=key,
...
def test_business_total_unchanged_when_seat_soft_deleted_mid_period():
    ...
    s1 = Customer.objects.create(tenant=t, external_id="alice", account_type="seat", parent=biz)
    s2 = Customer.objects.create(tenant=t, external_id="bob", account_type="seat", parent=biz)
    Posting.objects.create(tenant=t, customer=s1, idempotency_key="i1", provider_cost_micros=1, billed_cost_micros=800_000, effective_at=MID)
    Posting.objects.create(tenant=t, customer=s2, idempotency_key="i2", provider_cost_micros=1, billed_cost_micros=300_000, effective_at=MID)
...
def test_close_invoices_soft_deleted_seat_usage():
    ...
    ev = Posting.objects.create(tenant=t, customer=s1, idempotency_key="i1", provider_cost_micros=1, billed_cost_micros=500_000)
