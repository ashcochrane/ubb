...
**Nothing published moves.** The column is internal: written once, in
`postpaid_service._record`, read by this app's own tests and by nothing else. It
is declared in no schema and returned by no route, so `openapi/v1.json`
regenerates byte-identical and the contract's drift and breaking gates have
nothing to see. That is the whole reason a real column could be renamed in a
commit that otherwise moves only prose.
"""
