...
# monitor_ingest_health was here. It read the ingest-health module deleted
# alongside the ops route it served, so it could no longer run — and a beat
# entry whose dotted path still RESOLVES would not have been caught by the
# dangling-path check that inspects the schedule. It goes with its subject
# rather than waiting for the ticket that owns the rest of this module.
