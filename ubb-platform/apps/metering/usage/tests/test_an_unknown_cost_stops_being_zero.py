...
class ThePostingDeclaresWhatMayHappenToItsCostTest(TestCase):
    ...
    **Relaxing it to "the posting may declare classes" would have been the
    ...
    def test_every_defended_column_on_this_model_is_named_here(self):
        ...
        the point: it was slice 3's three until #352, five until #353, seven
        since #417, and each arrival had to come past a reader rather than
        alongside one. They did, and this is that reader — the assertion is
        unchanged in kind, and the set it names is the decision.

        ⚠ **THE NAME STOPPED LISTING THEM IN #417**, and that is the repair
        rather than laziness. *The two pairs, the claim and the receipt* was a
        running inventory in a test name: it went false the moment the `kind`
        discriminator was declared, silently, because nothing type-checks a
        list in a name — and a reader trusting it would have counted six where
        the assertion below names seven. The list is the claim; the name says
        what the claim is about.

        **Equality in both directions, deliberately.** A subset assertion would
        admit an eighth column nobody chose a class for, and a superset one
        ...
        rewritten around a new published shape does.
        """
