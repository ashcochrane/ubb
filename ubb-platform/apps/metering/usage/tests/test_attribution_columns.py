...
def test_tags_no_longer_lift_into_dim_columns():
    ...
    analytics labels only, never dimension attribution. dim1 still fills
    from the product_id request field (a separate wire-contract field, kept
    verbatim), but tags never touch any dim column."""
...
@pytest.mark.django_db
def test_explicit_product_id_still_fills_dim1_tag_does_not():
    t = Tenant.objects.create(name="T")
    c = Customer.objects.create(tenant=t, external_id="c1")
    r = UsageService.record_usage(t, c, "r1", "i1", provider_cost_micros=1000,
