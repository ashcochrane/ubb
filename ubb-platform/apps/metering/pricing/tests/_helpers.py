...
#: technique 1 of `docs/conventions/coding-standards.md`'s three, and the only
#: one available here, because putting the word once in this file would leave a
#: count of one where the entry needs none.
#:
...
#: went to the new entity rather than away. The third went nowhere, because the
#: split is what replaced it. So it is the removal whose column no cost book
#: has, and the one-element unpack is what fails loudly if that ever stops
#: picking out exactly one.
_SPLIT_MIGRATION = "0028_the_container_becomes_a_pricing_book_and_a_cost_book"


def _the_kind_word_the_split_deleted():
...
RETIRED_KIND_COLUMN = _the_kind_word_the_split_deleted()
