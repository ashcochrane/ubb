...
# task start, task close) is telemetry, not governance, and deliberately has NO
# action here — see the exemption list in api/v1/tests/test_audit_sweep.py.
# Registering a unit of work is the HEAD of that ingestion and closing one is
# its tail, which is why they sit together: neither changes the rules nor moves
# money. (The affordability question is a GET since #463 and authors nothing,
# so it needs no exemption either.)
AUDIT_ACTIONS = (
