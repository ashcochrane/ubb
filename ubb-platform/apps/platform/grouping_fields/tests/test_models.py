...
class TestGroupingField:
    ...
    def test_key_unique_per_tenant(self):
        t = self._t()
        GroupingField.objects.create(tenant=t, key="region", slot="dim1", scope="task")
        with pytest.raises(IntegrityError):
            GroupingField.objects.create(tenant=t, key="region", slot="dim2", scope="task")

    def test_slot_unique_per_tenant(self):
        t = self._t()
        GroupingField.objects.create(tenant=t, key="region", slot="dim1", scope="task")
        with pytest.raises(IntegrityError):
            GroupingField.objects.create(tenant=t, key="model", slot="dim1", scope="event")

    def test_same_key_allowed_across_tenants(self):
        a, b = self._t(), self._t()
        GroupingField.objects.create(tenant=a, key="region", slot="dim1", scope="task")
        GroupingField.objects.create(tenant=b, key="region", slot="dim1", scope="task")
