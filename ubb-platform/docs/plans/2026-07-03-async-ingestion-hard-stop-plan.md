...
- Endpoint algorithm (the hot path — no ORM per event beyond what is listed):
  1. `_product_check`; resolve tenant; gate on `"metering_async" in (tenant.products or [])` else 403 `feature_not_enabled`.
  2. Group items by customer (resolve customers once per batch, same memo-dict idiom as `record_usage_batch`); resolve billing owner once per customer.
  3. `CardCache.begin_request(tenant.id)`; per item: idem-check `SETNX ubb:idem:{tenant}:{customer}:{key}` (pipelined, TTL 7 days), estimate (`Unpriceable` → run item through `UsageService.record_usage` inline, mark `mode="sync_fallback"`).
  4. Run metadata for run items from a 30s L1 cache (`{run_id: (cap, seed_total, status)}`, single `Run.objects.values()` read on miss); killed/completed runs → item rejected `run_not_active`.
  ...
  7. `transaction.on_commit(lambda: settle_raw_events.delay())` then return verdicts.
