...
live catalogue with `ast` and never imports it. This module joins the two, and
subtracts the violations the migration ledger already owes — because the gate is
installed over a catalogue that today violates it twenty times, which is the
...
from .catalogue import Event, read_catalogue
...
def assess(repo_root):
    ...
    errors.extend(problems)
    #: A catalogue that could not be read holds no events, so every seeded debt
    #: would look paid. Reporting twenty "this debt is no longer owed" errors
    #: beside the one that says why is worse than useless — it buries the cause
    #: under twenty consequences, all of them false.
    read_failed = bool(problems)
    ...
    for value in sorted(vocabulary.declared_events):
        errors.extend(faults(
            value,
            f"{REGISTRY_DIR}/{registry.concepts['webhook_event_type'].source}"
            f"::{value}",
            vocabulary))
    ...
    for key in (sorted(excuses - owed) if not read_failed else ()):
