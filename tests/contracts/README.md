...
| `test_the_pin_ledger_names_tests_that_exist.py` | The same rule over a different document (#470): `docs/spend-control-guarantees.md` §9 invites a reader to *"run any row yourself"*, and three of its twenty-nine citations had gone stale under renames while every suite stayed green. Paths must be written from the git root, and pytest-style `Test*` classes are matched too — the shape the ADR walker's own carry-forward records as its blind spot. |
| `test_contract_suite_is_enforced.py` | This suite runs, is required, and needs no Django. |
| `_helpers.py` | Builds synthetic registries on disk for the negative controls, off the **real** schema — so a change to the shipped kind table is felt by every control. Also copies the **real** registry, for the controls that must mutate exactly one thing about it. Holds the reader the two document-walkers share: the names a module defines, the section a heading opens, and which of a document's citations fail to resolve. |
